...
타이머 `+(Ue?"O":"X")+"  감시 "+(Be?"O":"X")},400)}const un=.35,Ri=260,Pi=.08,Hi=.65,nt=Object.freeze([.25,.5,.75,1,1.25,1.5,2]),Bt="horo-playback-rate",ra=Number(b(Bt));let P=nt.includes(ra)?ra:1,de=[],oe=null,y=null,la=0,it=null;const pn=window.matchMedia("(prefers-color-scheme: dark)"),ca=b("horo-theme");let $t=ca==="dark"?"dark":ca==="light"?"light":pn.matches?"dark":"light";const pe=Ti({onPlayPause:ys,onFocusOpener:()=>{try{window.focus()}catch{}},onClosed:wn});function Mt(e=""){return`<button class="theme-toggle ${e}" type="button" data-theme-toggle></button>`}function In(e,t=!1){$t=e,document.documentElement.dataset.theme=e,document.querySelector('meta[name="theme-color"]').content=e==="dark"?"#151619":"#f5f5f7",t&&b("horo-theme",e),document.querySelectorAll("[data-theme-toggle]").forEach(n=>{const a=e==="dark"?"淺色模式":"深色模式";n.innerHTML=e==="dark"?Ys:ja,n.setAttribute("aria-label",`切換至${a}`),n.setAttribute("title",`切換至${a}`)}),he()}document.addEventListener("click",e=>{e.target.closest("[data-theme-toggle]")&&In($t==="dark"?"light":"dark",!0)});pn.addEventListener&&pn.addEventListener("change",e=>{b("horo-theme")||In(e.matches?"dark":"light")});function An(e=y,t=I){return e&&t==="jp"&&tt[e.id]||null}function gt(e=I){return qe[e]?.label||qe.jp.label}function yt(e=I){return zn[e]||zn.jp}function Ha(){return`<span class="chant-source-info">
  ...
  `,document.getElementById("back-btn").addEventListener("click",()=>{location.hash="#/"});const e=document.getElementById("guide-chant-source"),t=document.getElementById("guide-chant-source-content");e?.addEventListener("toggle",()=>{!e.open||!t||t.dataset.loaded==="1"||(t.innerHTML=`${Ha()}
      ...
      </section>`,t.dataset.loaded="1",Je())}),Je(),T.querySelectorAll(".song-sort").forEach(n=>{n.addEventListener("click",()=>{const a=n.dataset.sort;a==="chant"?N=N==="chant-desc"?"chant-asc":"chant-desc":a==="title"?N=N==="title"?"title-desc":"title":N=a,b("horo-song-sort",N),hn({eager:!0})})}),vo(),hn(),Ua(Vi(),[500],"#/guide")}let xe="guide";function vt(e){xe=e==="setlist"?"setlist":"guide"}function oo(){return xe==="setlist"?"#/setlist":"#/guide"}function kt(e){const t=A.find(n=>n.id===e.id);return t?t.title:e.title||e.id}function ke(e){return E(String(e??"")).replace(/\(([^()]*)\)/g,(t,n)=>`(<span lang="ja">${n}</span>)`)}let J="random",Ke=null,mn=!1,wt=0;function ro(){mn=!1,wt=0,J="random"}let Ka=(()=>{const e=parseInt(b("horo-set-pos"),10);return Number.isFinite(e)&&e>=0?e:-1})();function Ln(e){Ka=e,b("horo-set-pos",String(e))}function at(){const e=[];return ie.items.forEach(t=>t.songs.forEach((n,a)=>e.push({key:t.n+":"+a,id:n.id}))),e}function Ga(){const e=at().map(t=>t.key);for(let t=e.length-1;t>0;t--){const n=Math.floor(Math.random()*(t+1)),a=e[t];e[t]=e[n],e[n]=a}Ke=e,b("horo-set-shuffle2",JSON.stringify(e))}function lo(){const e=at();if(Ke&&Ke.length===e.length)return;const t=b("horo-set-shuffle2");if(t)try{const n=JSON.parse(t);if(Array.isArray(n)&&n.length===e.length){Ke=n;return}}catch{}Ga()}function za(){lo();const e=new Map(at().map(t=>[t.key,t]));return Ke.map(t=>e.get(t)).filter(Boolean)}function Ja(){return(J==="random"?za():at()).filter(t=>A.some(n=>n.id===t.id))}function co(e){if(e.songs.length===1){const t=e.songs[0],n=A.some(a=>a.id===t.id);return`
      <li class="set-item">
        <button class="set-row" data-id="${E(t.id)}" ${n?"":"disabled"}>
          <span class="set-num">${H(e.n)}</span>
          <span class="set-title">${ke(kt(t))}</span>
    ...
    <li class="set-item">
      <button class="set-row" data-id="${E(e.id)}" ${t?"":"disabled"}>
        ...
        <span class="set-title">${ke(kt(e))}</span>
        <span class="set-go">${t?"›":""}</span>
      </button>
    </li>`}function ot(){const e=document.getElementById("set-list");if(!e)return;const t=J==="order",n=at().length;e.innerHTML=t?ie.items.map(co).join(""):za().map(uo).join("");const a=document.getElementById("setlist-count");a&&(a.textContent=t?`東京/首爾 ${ie.items.length} 首・共 ${n} 首`:`共 ${n} 首`);const s=document.getElementById("setlist-mode-meta");s&&(s.textContent=t?"劇透":"隨機");const i=document.getElementById("set-shuffle");i&&(i.hidden=t);const o=document.getElementById("setlist-desc");o&&(o.innerHTML=t?`${E(ie.dates)}<br>點選歌曲即可前往應援指南`:`${E(ie.dates)}<br>目前將歌曲<b>打亂顯示，不公開演出順序</b>`);const r=document.getElementById("setlist-note");r&&(r.innerHTML=t?`※ 這是整理自粉絲紀錄的非官方東京/首爾歌單，首爾場順序與東京場相同；台北場實際演出順序仍以官方公告為準。<br>
       ※ 日期不同而有變化的曲目，已分為<b>六</b>・<b>日</b>。`:`※ 這是整理自粉絲紀錄的非官方東京/首爾歌單，首爾場順序與東京場相同；台北場實際演出順序仍以官方公告為準。<br>
       ※ 演出順序與日期差異曲目皆已隱藏。想查看完整內容，請在上方點選<b>演出順序</b>。`),T.querySelectorAll(".set-mode").forEach(c=>{const p=c.dataset.mode===J;c.classList.toggle("active",p),c.setAttribute("aria-pressed",p?"true":"false")});let l=0;e.querySelectorAll(".set-row").forEach(c=>{if(c.disabled)return;const p=l++;c.addEventListener("click",()=>{const d=A.find(u=>u.id===c.dataset.id);d&&(wt=window.scrollY,vt("setlist"),Ln(p),Pt(d))})}),Ua(ie.items.map(c=>Tn(kt(c))).join(""),[500],"#/setlist")}function po(){T.innerHTML=`
  ...
  `;const e=document.getElementById("karaoke-source-popover");let t=null;e&&e.addEventListener("toggle",()=>{t&&clearTimeout(t),e.open&&(t=setTimeout(()=>{e.removeAttribute("open"),t=null},2e3))}),document.getElementById("song-back-btn").addEventListener("click",()=>{location.hash=oo()}),document.getElementById("prev-song").addEventListener("click",()=>{St(-1)}),document.getElementById("next-song").addEventListener("click",()=>{St(1)}),document.getElementById("song-picker-btn").addEventListener("click",()=>{document.getElementById("song-sheet").classList.contains("open")?Ne():Eo()}),document.getElementById("song-sheet").querySelectorAll("[data-close-sheet]").forEach(o=>{o.addEventListener("click",Ne)}),document.getElementById("song-sheet-list").addEventListener("click",o=>{const r=o.target.closest(".song-sheet-item");if(!r)return;const l=A.find(c=>c.id===r.dataset.id);if(Ne(),!(!l||y&&l.id===y.id)){if(xe==="setlist"){const c=parseInt(r.dataset.pos,10);Number.isFinite(c)&&Ln(c)}Pt(l)}});const n=document.getElementById("sheet-search-input"),a=document.getElementById("sheet-search-clear");n&&(n.addEventListener("input",Et),a.addEventListener("click",()=>{n.value="",Et(),n.focus()}));const s=document.getElementById("tip-pic-card");s&&s.addEventListener("click",()=>{document.getElementById("tip-pic").classList.toggle("big")}),(function(){const r=S.querySelector(".lyrics-pane");if(!r)return;let l=0,c=0,p=!1;r.addEventListener("touchstart",d=>{if(!C||d.touches.length!==1){p=!1;return}const u=d.touches[0];if(u.clientX<24||u.clientX>window.innerWidth-24){p=!1;return}l=u.clientX,c=u.clientY,p=!0},{passive:!0}),r.addEventListener("touchend",d=>{if(!p)return;p=!1;const u=d.changedTouches&&d.changedTouches[0];if(!u)return;const m=u.clientX-l,g=u.clientY-c;Math.abs(m)<70||Math.abs(m)<Math.abs(g)*2||rn(m<0?1:-1)},{passive:!0})})(),document.getElementById("lyrics-list").addEventListener("click",o=>{const r=o.target.closest(".lyric-line");r&&(Sr(Number(r.dataset.time)),C&&(O=!1,es(Number(r.dataset.time))))}),document.getElementById("play-toggle").addEventListener("click",ys);const i=document.getElementById("pip-btn");i&&pe.supported&&(i.addEventListener("click",async()=>{await pe.open(bs())&&wn()}),wn()),document.getElementById("playback-rate").addEventListener("change",o=>{dr(Number(o.currentTarget.value))}),_e(),document.getElementById("chant-btn").addEventListener("click",()=>{pa(!C)}),document.getElementById("chant-off").addEventListener("click",()=>{pa(!1)}),document.getElementById("chant-prev").addEventListener("click",()=>{ua(-1)}),document.getElementById("chant-next").addEventListener("click",()=>{ua(1)}),document.getElementById("venue-btn").addEventListener("click",o=>{ce=!ce,b("horo-venue",ce?"1":""),Ot()}),document.getElementById("reading-btn").addEventListener("click",()=>{const o=dt.indexOf(j);j=dt[(o+1)%dt.length],b("horo-reading",j),ns(),ss()}),document.getElementById("japanese-toggle").addEventListener("click",()=>{K=!K,b("horo-show-japanese",K?"1":"0"),bn()}),document.getElementById("chinese-toggle").addEventListener("click",()=>{G=!G,b("horo-show-chinese",G?"1":"0"),bn()}),document.getElementById("karaoke-btn").addEventListener("click",()=>{_=!_,b("horo-karaoke",_?"1":"0"),as()}),document.getElementById("autoscroll-btn").addEventListener("click",o=>{Le=!Le,o.currentTarget.classList.toggle("active",Le),Le&&X(!0)}),it&&document.removeEventListener("keydown",it),it=o=>{if(!Y())return;const r=document.getElementById("song-sheet");if(o.key==="Escape"&&r.classList.contains("open")){o.preventDefault(),Ne();return}if(r.classList.contains("open")){if(o.key==="Tab"){const c=[...r.querySelectorAll("button, input")].filter(u=>!u.disabled&&u.offsetParent!==null),p=c[0],d=c[c.length-1];o.shiftKey&&document.activeElement===p?(o.preventDefault(),d.focus()):!o.shiftKey&&document.activeElement===d&&(o.preventDefault(),p.focus())}return}const l=(o.target&&o.target.tagName||"").toLowerCase();l==="input"||l==="textarea"||(o.key==="ArrowLeft"&&(o.preventDefault(),C?rn(-1):document.getElementById("prev-song").click()),o.key==="ArrowRight"&&(o.preventDefault(),C?rn(1):document.getElementById("next-song").click()))},document.addEventListener("keydown",it)}let C=!1,M=-1,O=!1,je=null,_t=0;function me(){return ir(y)}function So(e=y){const t=document.getElementById("lyrics-list");!t||!e||!Array.isArray(e.lyrics)||t.querySelectorAll(".lyric-line").forEach(n=>{const a=Number(n.dataset.idx),s=e.lyrics[a];if(!s)return;const i=Ss(s,e);n.classList.toggle("is-chant",i.split(" ").includes("is-chant")),n.classList.toggle("chant-partial",i.split(" ").includes("chant-partial")),n.classList.toggle("has-chant-segment",i.split(" ").includes("has-chant-segment"));const o=n.querySelector(".lyric-icons");o&&(o.innerHTML=ks(s,e))})}function Qa(e){if(e!=="jp"&&e!=="kr"||e===I){Je();return}I=e,b("horo-chant-version",I),Fe.clear(),mt.clear(),Ge.clear(),M=-1,O=!1,je=null,_t=0,Je(),y&&(So(y),Oa(y),Fa(y),Rt(),Ie(),C&&me().length?ee(0):Y()&&X(!0)),document.getElementById("song-list")&&hn()}function Rt(){const e=document.getElementById("song-page");e&&e.classList.toggle("chant-only",C&&me().length>0);const t=document.getElementById("chant-btn");t&&(t.classList.toggle("active",C),t.setAttribute("aria-pressed",C?"true":"false"))}function Ie(){const e=document.getElementById("chant-bar");if(!e||(e.hidden=!C,!C))return;const t=me(),n=document.getElementById("chant-msg"),a=document.getElementById("chant-count"),s=document.getElementById("chant-prev"),i=document.getElementById("chant-next");if(!t.length){n&&(n.textContent="這首歌沒有大合唱段落"),a&&(a.textContent=""),s&&(s.disabled=!0),i&&(i.disabled=!0);return}const o=window.matchMedia&&window.matchMedia("(hover: none)").matches;n&&(n.textContent=O?"結束・按播放即可從頭開始":o?"左右滑動切換段落":"用 ← → 切換段落"),a&&(a.textContent=`第 ${Math.max(1,M+1)} / ${t.length} 段`),s&&(s.disabled=M<=0),i&&(i.disabled=M>=t.length-1)}function ee(e){const t=me();if(t.length&&(M=Math.max(0,Math.min(t.length-1,e)),O=!1,es(t[M].start),Ie(),f&&typeof f.seekTo=="function"))try{f.seekTo(t[M].start,!0),f.playVideo()}catch{}}function es(e){je=e,_t=Date.now()+4e3}function ua(e){const t=me();if(t.length){if(O&&e<0){ee(t.length-1);return}ee(M+e)}}function Io(e){if(!C||e===null||!isFinite(e))return!1;const t=me();if(!t.length)return!1;const n=e-un;if(je!==null)if(Math.abs(n-je)<1.6||Date.now()>_t)je=null;else return!1;const a=t.findIndex(i=>n>=i.start-.35&&n<=i.end);if(a>=0)return(a!==M||O)&&(M=a,O=!1,Ie()),!1;const s=t.findIndex(i=>i.start>n);if(s>=0)return ee(s),!0;if(O=!0,M=t.length-1,Ie(),f&&typeof f.pauseVideo=="function")try{f.pauseVideo()}catch{}return!0}function pa(e){C=!!e,O=!1,M=-1,Rt(),Ie(),C?me().length&&ee(0):X(!0)}function rn(e){if(!Y())return;const t=me();if(C&&t.length){const n=M+e;if(e>0&&!O&&n<=t.length-1){ee(n);return}if(e<0&&O){ee(t.length-1);return}if(e<0&&n>=0){ee(n);return}}St(e)}function Pt(e){e&&(Ft(e.youtubeId),location.hash=`#/song/${e.id}`)}function Y(){return!S.classList.contains("offstage")}function Ht(e){const t=document.getElementById("song-page");if(!t)return;const n=Number($a[e?.id]);if(!Number.isFinite(n)||n<=0)return;const a=60/n/Math.max(.01,P);t.style.setProperty("--song-bpm",String(n)),t.style.setProperty("--icon-beat-duration",`${a.toFixed(3)}s`),t.dataset.bpm=String(n),Pa=performance.now(),he()}function ts(e){const t=++la;if(Za(),!Array.isArray(e.lyrics)){y=e;const{prev:c,next:p}=xt(e),d=document.getElementById("song-page");document.getElementById("song-page-heading").textContent=e.title,document.getElementById("song-picker-title").innerHTML=ke(e.title),document.getElementById("prev-song").title=`上一首：${c.title}`,document.getElementById("next-song").title=`下一首：${p.title}`,document.getElementById("lyrics-list").innerHTML="",document.getElementById("video-status").textContent="正在載入歌詞…",d?.classList.remove("song-ready"),S.classList.remove("offstage"),S.inert=!1,T.style.display="none",fi().then(u=>{if(t!==la||!location.hash.startsWith("#/song/"))return;const m=u.get(e.id);m&&ts({...e,...m})});return}mi().then(()=>{y===e&&ss()}),y=e,ve=-1,z=null,ge=null,M=-1,O=!1,je=null,_t=0,kr(),Ye(),oe!==null&&(cancelAnimationFrame(oe),oe=null);const{prev:n,next:a}=xt(e),s=document.getElementById("song-page");Ht(e),e.cover?s.style.setProperty("--song-cover",`url('${e.cover}')`):s.style.removeProperty("--song-cover"),document.getElementById("song-picker-title").innerHTML=ke(e.title),document.getElementById("song-page-heading").textContent=e.title;const i=document.getElementById("lyrics-credit");if(i){const c=!!e.translationCredit;if(c){const d=`<a href="${e.translationCreditUrl||Bi}" target="_blank" rel="noopener">${E(e.translationCredit)}</a>`,u=e.translationSourceUrl?` · <a href="${e.translationSourceUrl}" target="_blank" rel="noopener">出處</a>`:"",m=e.translationLicenseUrl?` · <a href="${e.translationLicenseUrl}" target="_blank" rel="noopener">${E(e.translationLicense||"授權")}</a>`:"";i.innerHTML=`中譯歌詞作者：${d}${u}${m}`}else i.innerHTML="";i.hidden=!c}const o=document.getElementById("song-back-btn");if(o){const c=xe==="setlist"?"返回歌單":"返回歌曲清單";o.title=c,o.setAttribute("aria-label",c)}document.getElementById("prev-song").title=`上一首：${n.title}`,document.getElementById("next-song").title=`下一首：${a.title}`,document.getElementById("watch-on-yt").href=`https://www.youtube.com/watch?v=${encodeURIComponent(e.youtubeId)}`,document.getElementById("lyrics-list").innerHTML=e.lyrics.map((c,p)=>`
    <li>
